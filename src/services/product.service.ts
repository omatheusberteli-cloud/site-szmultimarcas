...
export class ProductService {
  ...
  static async getProducts(
    ...
    try {
      ...
      // Transform products to include category info and normalize image fields
      const transformedProducts = (products || []).map((product: any) => ({
        ...product,
        category: product.categories?.slug || 'camisa',
        category_name: product.categories?.name || 'Camisa',
        imageUrl: product.images && product.images.length > 0 ? product.images[0] : null,
        stock: product.stock_quantity || 0
      }));

      return { products: transformedProducts, total: count || 0, error: null };
    } catch (error) {
      return { products: [], total: 0, error: error as Error };
    }
  }

  // Obter produto por ID com variações
  static async getProductById(productId: string): Promise<{ product: Product | null; variations: ProductVariation[]; error: Error | null }> {
    try {
      ...
      // Normalize product data to include imageUrl and stock
      ...
      return { product: normalizedProduct, variations: variations || [], error: null };
    } catch (error) {
      return { product: null, variations: [], error: error as Error };
    }
  }
  ...
  // Reduzir estoque por variação (tamanho + cor)
  static async reduceStockByVariation(
    variationId: string,
    quantity: number
  ): Promise<{ variation: ProductVariation | null; error: Error | null }> {
    try {
      // Primeiro obtém a variação atual
      ...
      const newQuantity = Math.max(0, currentVariation.stock_quantity - quantity);
      ...
      return { variation, error: null };
    } catch (error) {
      return { variation: null, error: error as Error };
    }
  }

  // Criar variação de produto
  static async createVariation(data: ProductVariationInsert): Promise<{ variation: ProductVariation | null; error: Error | null }> {
    try {
      const { data: variation, error } = await supabase
        .from('product_variations')
        .insert(data)
  ...
  static async deleteVariation(variationId: string): Promise<{ error: Error | null }> {
    try {
      const { error } = await supabase
        .from('product_variations')
        .delete()
