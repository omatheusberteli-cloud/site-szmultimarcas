...
export interface CartItem {
  ...
  productId: string;
  variationId?: string; // New field for variation tracking
  name: string;
  price: number;
  imageUrl: string;
  category: string;
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = (product: Product, size?: string, color?: string, quantity = 1) => {
    ...
    const selectedColor = color || 'Preto';
    const itemId = `${product.id}-${selectedSize}-${selectedColor}`;

    // Note: Stock reduction should be handled when selecting a specific variation
    // The new structure uses variationId which needs to be passed from ProductDetail
